...
internal class SharedValidationGraph(artifacts: MergedArcArtifacts) {
    val declarations = artifacts.sharedValidators.groupBy { it.model }.mapValues { (_, values) -> values.sortedBy { it.identity } }
    ...
    init {
        ...
        do {
            ...
            (properties + interfaces.mapValues { it.value.properties }).forEach { (name, members) ->
                if (name !in active && (members.any { reaches(it.shape) || it.derivatives.any(active::contains) } ||
        ...
        if (declarations.isNotEmpty()) {
            ...
            fun visit(name: String, path: String) {
                ...
                if (visited.add(name)) properties[name].orEmpty().forEach { member ->
            ...
            fun inspect(shape: TypeShapeDescriptor, path: String) {
                leaves(shape).forEach { name ->
                    ...
                    if (checked.add(name)) properties[name].orEmpty().forEach { inspect(it.shape, "$path.${it.name}") }
                }
            }
            artifacts.commands.forEach { command -> command.properties.forEach { inspect(it.shape, "${command.typeName}.${it.name}") } }
