...
| 9 | Interface default methods | `Source` sets `JvmDefaultMode.ENABLE`, preserving Java default methods and `DefaultImpls` compatibility classes. Other modules inherit their compiler defaults unless configured explicitly; inspect emitted methods and forwarding bridges rather than assuming abstract members |
