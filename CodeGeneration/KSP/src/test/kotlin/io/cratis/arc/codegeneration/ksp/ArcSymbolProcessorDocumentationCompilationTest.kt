...
internal class ArcSymbolProcessorDocumentationCompilationTest {
    ...
    fun `generated Kotlin carries documentation that would otherwise break a string literal`() {
        ...
        val metadata = generatedMetadataSource(directory)
        assertTrue(
            "summary = \"Creates a documented fixture.\"" in metadata,
            "The generated metadata factory must carry the command summary:\n$metadata"
        )
        assertTrue("summary = \"Documented command key.\"" in metadata, metadata)
        assertTrue("summary = \"Finds a documented model.\"" in metadata, metadata)
        assertTrue("Documented client filter that costs" in metadata, metadata)
        // The dollar sign must be escaped, or the generated Kotlin would not have compiled at all.
        assertTrue("costs \\\$5" in metadata, metadata)
    }
    ...
    private fun generatedMetadataSource(directory: File): String {
        val root = directory.toPath().resolve("ksp/sources/kotlin/io/cratis/arc/generated")
        return Files.list(root).use { paths ->
            val metadata = paths.filter(Files::isRegularFile)
                .filter { path -> path.fileName.toString().endsWith("ArcArtifactMetadata.kt") }
                .toList()
                .single()
