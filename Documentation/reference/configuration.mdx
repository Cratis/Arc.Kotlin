...
| `proxies.useProxyFileSuffix` | `false` | Names generated type files `Name.proxy.ts` instead of `Name.ts`. |
| `proxies.typeMappings` | Empty | Raw `<FullyQualifiedTypeName>=<TypeScriptType>[=<NpmPackage>]` entries, consulted before the built-in type map. Prefer `proxies.mapType(typeName, typeScriptType)` or `proxies.mapType(typeName, typeScriptType, npmPackage)`. |
| `proxies.packageMappings` | Empty | JVM package to npm package mappings; model types under a mapped package are imported instead of generated, and the longest matching package wins. Prefer `proxies.mapPackage(javaPackage, npmPackage)`. |

The [TypeScript proxy options reference](typescript-proxy-options.md) compares each option with the Arc .NET generator.
...
<ArcBackendTabs snippet="reference/configuration/configure-object-mapper" variants="kotlin,java" />
