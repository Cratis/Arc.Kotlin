...
A command body is handed straight to the injected Jackson 2 `ObjectMapper`. The Spring Boot starter contributes `ArcJacksonModule` and the `arcJacksonCustomizer` bean, implemented as a Jackson-2-only `BeanPostProcessor`, which sets the naming strategy, null inclusion, named floating-point values, and date/duration text — and neither touches unknown-property handling. A hosted application therefore inherits Spring Boot's relaxed default, and `{"value":"hello","unexpected":"extra"}` executes the command with `unexpected` discarded. This matches Arc .NET, which does not set `UnmappedMemberHandling` either and so lets `System.Text.Json` skip unmapped members.

Spring Boot 4 also places Jackson 3 on the application classpath and uses it for conventional MVC controllers by default. The two mappers coexist because their root packages differ (`com.fasterxml.jackson` and `tools.jackson`). Arc handlers write their own responses with the injected Jackson 2 mapper, so their wire contract is unchanged. Set `spring.http.converters.preferred-json-mapper=jackson2` only when conventional application controllers should share that same mapper; the Arc starter does not make this application-wide choice.
