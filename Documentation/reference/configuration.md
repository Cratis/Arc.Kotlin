...
Published Kotlin APIs use checked `.api` baselines. The root `apiCheck` gate covers `arc`, KSP, the Gradle plugin implementation artifact, every published integration, and testing support; contract tests and runnable samples are intentionally unpublished and excluded. An API change therefore requires an explicit baseline update rather than silently changing compiler or plugin contracts. The temporal/UUID proxy slice changes generated TypeScript source types. Recursive shape metadata and source summaries were introduced in manifest format 6. Current manifest format 7 adds optional typed `eventMetadata` to commands while preserving immutable public JVM shape descriptors, legacy constructor descriptors, and compatibility getters. Because the reader accepts exactly the current version, a dependency built against an earlier format must be rebuilt.
