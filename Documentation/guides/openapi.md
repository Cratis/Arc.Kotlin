...
Generated components cover command payloads, client-supplied query parameters, model, interface, and enum metadata, nullable and collection types, and Arc's `CommandResult`, `QueryResult`, `ValidationResult`, `PagingInfo`, and `ChangeSet` envelopes. Service, `QueryRequest`, and `QueryContext` parameters are runtime infrastructure and are omitted from OpenAPI. Enum schemas use their numeric Arc wire values and expose names through `x-enumNames`.

Property keys and `required` entries use Arc's default JSON and TypeScript naming rule: `Title` becomes `title`, while names beginning with two uppercase letters, such as `URLValue`, stay unchanged. Nullable properties are non-required and allow JSON null; Arc's default mapper omits null properties. This describes the default Arc wire policy, not arbitrary application mapper, mixin, or annotation naming overrides. Supply an application-owned document when your wire policy differs.

Generated Kotlin and Java interface descriptors receive reusable object components containing their declared metadata fields, and interface-valued properties and collection items reference those components. Simple names are shared across models, interfaces, enums, concepts, and commands; collisions and Arc's reserved envelope names use qualified component names instead. Base-class model schemas retain `allOf` composition rather than flattening inherited properties. Interface and derived-model schemas describe only the structure represented in generated metadata: they do not invent complete `oneOf` alternatives, discriminators, or missing inheritance information.
