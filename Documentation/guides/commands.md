...
For constructor-authored rules shared with generated TypeScript, follow the [fluent validation walkthrough](validation.md), including [direct validation, preflight and execution](validation.md#execute-and-inspect-rejection). Generated modules register these rules automatically, including supported nested models. Ordinary imperative validators and Jakarta remain separate; neither is executed by build tooling.
...
Exclusions are server-only registrations, not annotations or client rules. They do not independently change the manifest format, generated Jakarta metadata, TypeScript constraints, or OpenAPI schemas. The separate [IgnoreValidation annotation](../reference/validation.md#ignore-a-validation-member-edge) has a broader whole-edge contract and explicit format-8 metadata. Generated Kotlin and Java scenario/HTTP checks cover the concept/model boundary; HTTP query model checks install an application string-to-model converter rather than claiming built-in arbitrary object binding. The Spring hosting checks separately use its real Jakarta provider to prove excluded edges still reject constraint violations. Neither establishes Arc .NET parity.
