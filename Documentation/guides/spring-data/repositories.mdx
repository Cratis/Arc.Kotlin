...
`TenantContextBridge.withTenant` blocks its calling thread, so it is for plain Java threads; inside a Java
command handler, which Arc already runs in a coroutine, use the explicit-tenant overload below instead.
