...
A Java handler reads the same ambient tenant through `operationsForCurrentTenant()`. On a plain
thread with no Arc call above it, establish the tenant first with `TenantContextBridge.withTenant`.
