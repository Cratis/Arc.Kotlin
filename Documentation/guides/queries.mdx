...
<ArcBackendTabs snippet="guides/queries/paths-and-services" variants="kotlin,java" />
...
- WebSocket is selected by upgrading the query route. It needs Spring WebSocket on the application classpath, because the starter declares it only at compile time: add `org.springframework.boot:spring-boot-starter-websocket` (or `org.springframework:spring-websocket`). Set `cratis.arc.observable-queries.web-socket-enabled=false` to disable these routes; the default is `true`. If Spring WebSocket is absent or this property is `false`, the WebSocket routes, including `/.cratis/queries/ws`, are not registered and SSE remains available. Data frames use `{ "type": "Data", "data": {QueryResult} }`; `Ping` and `Pong` frames carry millisecond timestamps.
