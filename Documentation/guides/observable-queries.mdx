...
snapshot `GET` from the source's current value and, by default, reports a source without one as
`202 Not Ready` rather than holding the request open. A caller can pass `waitForFirstResult=true`
to wait up to the configured limit instead.
