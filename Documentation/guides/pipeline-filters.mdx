...
A filter sees the resolved [`CommandContext`](commands.mdx) — the command instance, its type, the
principal authentication already produced, the correlation identifier, the tenant — and returns a
result that is merged into the outcome. An unsuccessful result stops the remaining filters. It stops
the handler too when the result carries an authorization failure, an exception, or validation
feedback that is still blocking after the allowed-severity threshold is applied; warning-only
feedback is dropped by default and the handler runs:
...
This example is an ordinary filter, so it runs in `@Order` sequence with the others. Because its job
is to authorize, make it an `AuthorizationCommandFilter` (in Java, a
`BlockingAuthorizationCommandFilter` registered through `BlockingAuthorizationCommandFilterAdapter`)
when it must run before filters that cause effects. [Order them deliberately](#order-them-deliberately)
explains why.
...
A filter whose job is to authorize has its own variant of each interface. Its adapter keeps it in
the authorization group that runs first:

- `BlockingAuthorizationCommandFilter`, registered with `BlockingAuthorizationCommandFilterAdapter`
- `BlockingAuthorizationQueryFilter`, registered with `BlockingAuthorizationQueryFilterAdapter`
- `AsyncAuthorizationCommandFilter`, registered with `AsyncAuthorizationCommandFilterAdapter`
- `AsyncAuthorizationQueryFilter`, registered with `AsyncAuthorizationQueryFilterAdapter`
...
apply. See [authentication and authorization](security.mdx).
