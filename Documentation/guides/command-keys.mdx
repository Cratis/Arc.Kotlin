...
<ArcBackendTabs snippet="guides/command-keys/declared-key" variants="kotlin,java" />
...
<ArcBackendTabs snippet="guides/command-keys/computed-key" variants="kotlin,java" />
...
role in resolving command state are the same. The types differ: C#'s `GetKey()` returns a string,
while the JVM `CommandKeyProvider.commandKey()` returns `Any?`, so each integration decides which key
types it accepts. The Chronicle integration accepts `String`, `UUID`, a number, or a concept wrapping
one of those. The [parity reference](../reference/parity.md) tracks the exact status.
