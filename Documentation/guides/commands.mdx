...
<ArcBackendTabs snippet="guides/commands/handle" variants="kotlin,java" />
...
<ArcBackendTabs snippet="guides/commands/provide" variants="kotlin,java" />
...
<ArcBackendTabs snippet="guides/commands/response-values" variants="kotlin,java" />
...
For constructor-authored rules shared with generated TypeScript, follow the [fluent validation walkthrough](validation.mdx), including [direct validation, preflight and execution](validation.mdx#execute-and-inspect-rejection). Generated modules register these rules automatically, including supported nested models. Ordinary imperative validators and Jakarta remain separate; neither is executed by build tooling.

Register a Spring bean implementing `CommandValidator<T>`. Java implements `BlockingCommandValidator<T>` and registers it through `BlockingCommandValidatorAdapter`, because Arc collects only `CommandValidator` beans — a bean implementing the blocking interface on its own is never run. Return immutable validation results; an error rejects execution. `POST <command-route>/validate` runs the same command pipeline without invoking the handler.

<ArcBackendTabs snippet="guides/commands/validator" variants="kotlin,java" />
...
<ArcBackendTabs snippet="guides/commands/validation-failure" variants="kotlin,java" />
...
Authorization can be declared on the artifact class or on its operation — the command `handle` function or a read-model query function. **The operation wins.** When the operation declares any `@Authorize` or `@Roles`, only its policy, roles, and schemes apply and the class declaration is ignored entirely; the class declaration applies only to operations that declare none. An operation therefore replaces the class rule rather than adding to it, and it can admit callers the class rule would reject: on a class requiring `admin`, a `@Roles("auditor")` operation is reachable by auditors only, never by admins. Repeated `@Roles` on the *same* target still combine, and a caller satisfies a role list by holding any one of its roles.
