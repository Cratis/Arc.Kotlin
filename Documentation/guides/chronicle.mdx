...
:::note[The in-process command scenario is Kotlin-only]
`CommandScenario.execute` is a suspend function with no blocking or `CompletionStage`
counterpart, so a Java test cannot call it. Java applications test these commands over HTTP
instead. This is the one example on this page with no Java form, because none exists.
:::
