...
Run either sample against a Chronicle 18.4.0 or newer kernel and include `x-cratis-tenant-id` on every request. `Samples/run.sh --chronicle` starts the `cratis/chronicle:18.4.0-development` image for you. The explicit compatibility gate starts its own pinned development image and exercises both applications:

```shell
./gradlew :ContractTests:chronicleRealKernelTest --no-configuration-cache
```

Override `-PchronicleKernelImage=<pinned-image>` only for a deliberate compatibility run. The default image is `cratis/chronicle:19.1.2-development` pinned to digest `sha256:8477bf1622813730dfde0787b688f4ed6698b9ca45032163813f26f429235a38`. Docker absence fails this explicit task; normal unit tests do not start a kernel.
