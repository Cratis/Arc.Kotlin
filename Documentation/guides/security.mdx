...
`@AllowAnonymous` together with `@Authorize` or `@Roles` **on the same class or the same operation** is a contradiction with no defensible reading. KSP reports `ARCKSP0108` and stops generation rather than guessing. Splitting them across a class and its operation is a different thing entirely, and is exactly the override above. Declaring `@AllowAnonymous` on both the class and its operation is also `ARCKSP0108`: it is redundant, so declare it once.
:::
...
- `GET /.cratis/commands`
- `GET /.cratis/queries`
- `GET /.cratis/users` — see [Development users and tenants](development-users-and-tenants.md)
- `GET /.cratis/tenants` — see [Development users and tenants](development-users-and-tenants.md)
