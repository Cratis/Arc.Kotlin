...
[Fluent validation](validation.mdx) contributes verified declarations to the existing renderer. Active models export `<Model>Validator`; command and supported RFC QUERY validators compose nested shared rules, including indexed siblings. Structurally equal duplicate model descriptors merge validation conjunctions; incompatible structures reject. Annotation-only output and its differential boundary are unchanged. Manual shared-generation CLI calls require `--module-name` and the complete compiled root/dependency classpath, not only manifest resources. Follow the [generated excerpt and handwritten caller](validation.mdx#display-generated-client-feedback), or consult the [dependency extraction and root inventory contract](../reference/validation.md#manual-build-tooling).

## Configure generation

Use the `io.cratis.arc` plugin and set an output directory. Route configuration exists on both sides — build-time (`cratisArc.endpoints.*`) and runtime (`cratis.arc.endpoints.*`) — and both sides must agree. The Arc Spring Boot starter enforces this automatically. The plugin's `writeArcEndpointOptionsResource` task packages the build-time settings as `META-INF/arc/endpoint-options.json` on the main resources, whether or not proxy generation runs. At startup the starter reads that resource and fails with an error that names the disagreeing setting and both values if they differ. The check is skipped only when `META-INF/arc/endpoint-options.json` is absent from the application classpath. Builds without the Arc Gradle plugin can package the same resource through the proxy CLI's `--endpoint-options-output` option, described below.
...
<ArcBackendTabs snippet="guides/typescript-proxies/documented-command" variants="kotlin,java" />
