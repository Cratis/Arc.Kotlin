#!/usr/bin/env bash
# Run the Arc.Kotlin Java + Chronicle task-board sample.
#
# Usage:
#   ./run.sh [--no-docker]
#
...
if "$USE_DOCKER"; then
    start_chronicle_kernel "arc-java-chronicle-sample-kernel"
fi

echo "▶  Running the Java + Chronicle sample on http://localhost:8080"
echo ""
cd "$REPO_ROOT"
