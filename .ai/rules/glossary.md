...
| **Manifest** | `ArcArtifactManifest` — the language-neutral JSON description of one module's artifacts, written to `META-INF/cratis/arc/<module>.json` and read by the proxy generator. Its contract version is `CURRENT_FORMAT_VERSION`, declared in the source. | `Source/artifacts/ArcArtifactManifest.kt` |
